...
export async function GET(req: Request) {
  ...
  if (!questionId) return NextResponse.json({ items: [] }, { headers: { "Cache-Control": NO_STORE } });
...
export async function POST(req: Request) {
  ...
  if (!questionId || !text) {
    return NextResponse.json({ error: "missing questionId or text" }, { status: 400, headers: { "Cache-Control": NO_STORE } });
  }
  const a: Answer = {
  ...
  await addAnswer(a);
  return NextResponse.json({ ok: true, answer: a }, { headers: { "Cache-Control": NO_STORE } });
}

export async function DELETE(req: Request) {
  const url = new URL(req.url);
  const questionId = url.searchParams.get("q");
  if (!questionId) return NextResponse.json({ error: "missing q" }, { status: 400, headers: { "Cache-Control": NO_STORE } });
  await clearAnswers(questionId);
  return NextResponse.json({ ok: true }, { headers: { "Cache-Control": NO_STORE } });
}
